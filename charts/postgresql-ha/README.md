...
This Helm chart has been developed based on [qingcloud/postgresql](https://github.com/qingcloud/charts/tree/master/qingcloud/postgresql) chart but including some changes to guarantee high availability such as:
...
- Replacing `qingcloud/postgresql` with `qingcloud/postgresql-repmgr` which includes and configures [repmgr](https://repmgr.org/). Repmgr ensures standby nodes assume the primary role when the primary node is unhealthy.

## TL;DR

```console
$ helm repo add qingcloud https://charts.qingcloud.com/qingcloud
$ helm install my-release qingcloud/postgresql-ha
```
...
| `postgresqlImage.repository`                    | Repository for PostgreSQL with Repmgr image                                                                                                                                                     | `qingcloud/postgresql-repmgr`                                  |
...
| `pgpoolImage.repository`                        | Repository for Pgpool                                                                                                                                                                           | `qingcloud/pgpool`                                             |
...
| `metricsImage.repository`                       | Repository for PostgreSQL Prometheus exporter                                                                                                                                                   | `qingcloud/postgres-exporter`                                  |
...
| `volumePermissionsImage.repository`             | Init container volume-permissions image repository                                                                                                                                              | `qingcloud/qingcloud-shell`                                      |
...
### [Rolling VS Immutable tags](https://docs.qingcloud.com/containers/how-to/understand-rolling-tags-containers/)

It is strongly recommended to use immutable tags in a production environment. This ensures your deployment does not change automatically if the same tag is updated with a different image.

Qingcloud will release a new chart updating its containers if a new version of the main container, significant changes, or critical vulnerabilities exist.

### Change PostgreSQL version

To modify the PostgreSQL version used in this chart you can specify a [valid image tag](https://hub.docker.com/r/qingcloud/postgresql-repmgr/tags/) using the `image.tag` parameter. For example, `image.tag=X.Y.Z`. This approach is also applicable to other images like exporters.
...
The [Qingcloud PostgreSQL with Repmgr](https://github.com/qingcloud/qingcloud-docker-postgresql-repmgr) image allows you to use your custom scripts to initialize a fresh instance. You can specify custom scripts using the `initdbScripts` parameter as dict so they can be consumed as a ConfigMap.
...
+info: https://github.com/qingcloud/qingcloud-docker-postgresql#initializing-a-new-instance and https://github.com/qingcloud/qingcloud-docker-pgpool#initializing-with-custom-scripts
...
As an alternative, you can use of the preset configurations for pod affinity, pod anti-affinity, and node affinity available at the [qingcloud/common](https://github.com/qingcloud/charts/tree/master/qingcloud/common#affinities) chart. To do so, set the `XXX.podAffinityPreset`, `XXX.podAntiAffinityPreset`, or `XXX.nodeAffinityPreset` parameters.

## Troubleshooting

Find more information about how to deal with common errors related to Qingcloud’s Helm charts in [this troubleshooting guide](https://docs.qingcloud.com/general/how-to/troubleshoot-helm-chart-issues).
...
```bash
$ helm upgrade my-release qingcloud/postgresql-ha \
...
```bash
$ helm upgrade my-release --version 5.2.0 qingcloud/postgresql-ha \
    --set postgresql.password=[POSTGRESQL_PASSWORD] \
    --set postgresql.repmgrPassword=[REPMGR_PASSWORD] \
...
$ helm install my-release \
    --set postgresql.password=[POSTGRESQL_PASSWORD] \
    --set postgresql.repmgrPassword=[REPMGR_PASSWORD] \
    qingcloud/postgresql-ha --version 3.9.1
...
$ helm install my-release \
    --set postgresql.password=[POSTGRESQL_PASSWORD] \
    --set postgresql.repmgrPassword=[REPMGR_PASSWORD] \
    qingcloud/postgresql-ha --version 5.0.0
```
...
```bash
$ helm upgrade my-release --version 3.0.0 qingcloud/postgresql-ha \
    --set postgresql.password=[POSTGRESQL_PASSWORD] \
    --set postgresql.repmgrPassword=[REPMGR_PASSWORD] \
    --set postgresql.replicaCount=1 \
    --set postgresql.upgradeRepmgrExtension=true
...
```bash
$ helm upgrade my-release --version 1.0.0 qingcloud/postgresql-ha \
    --set postgresql.password=[POSTGRESQL_PASSWORD] \
    --set postgresql.repmgrPassword=[REPMGR_PASSWORD] \
    --set postgresql.replicaCount=1 \
    --set postgresql.upgradeRepmgrExtension=true
...
## Qingcloud Kubernetes Documentation

Qingcloud Kubernetes documentation is available at [https://docs.qingcloud.com/](https://docs.qingcloud.com/). You can find there the following resources:

- [Documentation for PostgreSQL HA Helm chart](https://docs.qingcloud.com/kubernetes/infrastructure/postgresql-ha/)
- [Get Started with Kubernetes guides](https://docs.qingcloud.com/kubernetes/)
- [Qingcloud Helm charts documentation](https://docs.qingcloud.com/kubernetes/apps/)
- [Kubernetes FAQs](https://docs.qingcloud.com/kubernetes/faq/)
- [Kubernetes Developer guides](https://docs.qingcloud.com/tutorials/)
